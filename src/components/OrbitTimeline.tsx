...
export default function OrbitTimeline({ currentPhase, completedPhases, phaseNames, onPhaseClick }: OrbitTimelineProps) {
  return (
    <div className="w-full bg-cosmic-dark border-b border-cosmic-border px-2 sm:px-4 py-2 sm:py-3 relative overflow-hidden flex-shrink-0">
      {/* Orbital track line */}
      ...
      <div className="overflow-x-auto scrollbar-none relative">
        <div className="flex justify-between items-center max-w-6xl mx-auto min-w-[640px] sm:min-w-0 px-1 sm:px-0">
          {Array.from({ length: 8 }, (_, i) => i + 1).map((phase) => {
            ...
            return (
              <motion.div
                key={phase}
                className="flex flex-col items-center cursor-pointer relative z-10 flex-shrink-0"
              ...
              >
                ...
                <motion.div
                  className={`
                    w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-sm font-bold font-mono
                    ...
                    ${!isCompleted && !isCurrent ? 'bg-cosmic-panel border-cosmic-border text-cosmic-muted hover:border-cosmic-accent/50' : ''}
                  `}
                  animate={isCurrent ? {
                    boxShadow: [
                ...
                >
                  <span className="text-sm sm:text-lg">{phaseIcons[phase]}</span>
                </motion.div>

                {/* Phase number */}
                <span className={`
                  text-[8px] sm:text-[10px] font-mono mt-1 sm:mt-1.5 tracking-wider
                ...
                {/* Phase name tooltip */}
                <span className={`
                  text-[7px] sm:text-[9px] font-sans mt-0.5 hidden sm:block max-w-[70px] text-center leading-tight
                ...
                {isCompleted && (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="absolute -top-1 -right-1 w-4 h-4 sm:w-5 sm:h-5 bg-cosmic-success rounded-full flex items-center justify-center text-[8px] sm:text-[10px]"
                  >
                    ✓
                  </motion.div>
                )}
